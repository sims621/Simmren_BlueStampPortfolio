...
For my final milestone, I built the robot chassis and completed all the wiring for the motors. I also initiated several tests and tweaked the code until my robot was functional. I also spent time troubleshooting issues with the wiring, motor controls, and camera calibration until everything worked smoothly. Finally, I designed and 3D printed a case for my robot as my modification using Fusion software. The case is able to hold my robot's power bank, raspberry pi, and wiring. Now, the robot is able to succesfully follow the ball wherever the ball is going.  Overall, I was able to combine artificial intelligence, programming, and engineering to create a fully functional robot that successfully tracks and follows a red ball. This project taught me valuable skills in coding, electronics, problem-solving, and robotics, and it gave me a better understanding of how computer vision can be used in real-world applications.
...
My first milestone consisted of me beginning to work on my Ball Tracking robot. The robot uses a Raspberry Pi and a camera to recognize the red ball. During my demonstration, I ran a Python program called cam_test.py, which displayed a live camera feed, a red mask, and a bounding box around the ball. The red mask filters out everything except the color red, making it easier for the computer to detect the ball. When the ball is visible, the program identifies its location, and when it is out of view, it displays "No ball found." The Raspberry Pi processes this information and sends commands to the robot's motors. For example, if the ball is on the right side, the robot will turn right, and if it is on the left, it will turn left. I also plan to use ultrasonic sensors to help the robot navigate safely while following the ball. One of the biggest challenges I faced was connecting the Raspberry Pi to my computer for remote access. After several unsuccessful attempts, I solved the problem by connecting it directly to a monitor and using the terminal and VNC. My next steps are to build the robot's chassis, connect the motors and wiring, test the complete system, and add additional features to improve its performance. Overall, this project has helped me gain experience with programming, electronics, problem-solving, and artificial intelligence while creating a robot that can intelligently track and follow an object.
